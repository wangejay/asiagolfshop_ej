	...
	if (isset($_POST['ua_wdm_add_auc']) && wp_verify_nonce($_POST['ua_wdm_add_auc'], 'ua_wp_n_f')) {
		...
		if (($auc_end_tm > $blog_curr_tm) && $auction_title != "" && $auction_content != "") {
			...
			if ($post_id) {
				...
				/**
				 * Update Digital product File
				 */

				if (isset($_POST['digital_auction_file'])) {
					update_post_meta($post_id, 'wdm-digital-product-file', $_POST["digital_auction_file"]);
				}
...
if (!empty($bidding_engine)) {
	...
	if ((get_option("wdm_default_bidding_engine_set") == "Yes") || (get_option("wdm_default_bidding_engine_set") == "No" && get_option("wdm_bidding_engines") == "")) {
		$after_thumb .= '<option value="">' . __("Simple Bidding", "wdm-ultimate-auction") . '</option>';
	}
...
//if($default_product_type != 'both')
    $product_type = $this->wdm_post_meta('wdm_product_type');
...
if (empty($product_type)) {
	$product_type = $default_product_type;
		...
		?>
		<th scope="row">
		    <label for="wdm_product_type"><?php _e("Product Type", "wdm-ultimate-auction");?></label>
		</th>
		<td>
		    <select class='wdm_settings_input' id='wdm_product_type' name='wdm_product_type'>
			<option value='physical' <?php echo $product_type == "physical"?"selected='selected'":"";?>><?php echo __('Physical', 'wdm-ultimate-auction');?></option>
			<option value='digital' <?php echo $product_type == "digital"?"selected='selected'":"";?>><?php echo __('Digital', 'wdm-ultimate-auction');?></option>
	        </select>
	    </td>
	</tr>
	...
	elseif($product_type=='physical' || $product_type=='digital')
		{
		     ?>
		    <input type="hidden" id="wdm_product_type" value="<?php echo($product_type);?>" name="wdm_product_type"/>
		    <?php
		}

	//$hidden = $product_type=="digital"?"":"hidden";
$single_img = $this->wdm_post_meta("wdm-digital-product-file");
if ($default_product_type == 'both' || $product_type === 'digital')
{
echo '<tr valign="top" id="wdm_digital_product_file_li">
			<th scope="row">
				<label for="digital_auction_file">' . __("Product File", "wdm-ultimate-auction") . '</label>
			</th>
			<td>
				<input name="digital_auction_file" type="text" id="digital_auction_file" class="regular-text digital_auction_file_url" value="' . $single_img . '"/>
				<input name="digital_auction_image_button" id="digital_auction_file_url" class="button wdm_auction_image_upload" type="button" value="' . __('Select File', 'wdm-ultimate-auction') . '"/>
			</td>
		    </tr>';
}
?>
...
?>
        ...
        <td>
            <select id="auction_main_image" name="auction_main_image">
                <?php for ($m = 1; $m <= 4; $m++) {
	?>
				<option value="main_image_<?php echo $m;?>" <?php echo $this->wdm_post_meta("wdm-main-image") == "main_image_" . $m ? "selected" : "";?>><?php _e("Product Image/Video", "wdm-ultimate-auction");
	echo " " . $m;?></option>
				<?php
}
?>
...
?>
    ...
    <tr valign="top">
        ...
        <tr valign="top">
        <th scope="row">
            <label for="ua_best_offers"><?php _e("Best Offers", "wdm-ultimate-auction");?></label>
        </th>
        <td>
            <input name="ua_best_offers" type="checkbox" id="ua_best_offers" value="1" <?php checked('1', $this->wdm_post_meta('wdm_enable_best_offers'));?> /><?php _e(" Enable", "wdm-ultimate-auction");?>   <!-- Enable/Disable Best Offers -->
        </td>
    </tr>
    <?php do_action('ua_add_shipping_cost_input_field'); //hook to add new price field
    
    $attach_label = $this->wdm_post_meta("product_attachment_label");
    $attach_file = $this->wdm_post_meta("wdm_product_attachment");
    ?>
    <tr valign="top">
        <th scope="row">
            <label for="wdm_attach_file"><?php _e("Product Attachment", "wdm-ultimate-auction");?></label>
        </th>
	<td>
	    <input name="product_attachment_label" type="text" id="product_attachment_label" value="<?php echo $attach_label;?>" />	
	    <input name="wdm_product_attachment" type="text" id="wdm_product_attachment" class="regular-text wdm_attachment_url" value="<?php echo $attach_file;?> "/>
	    <input name="wdm_attach_file" id="wdm_attachment_url" class="button wdm_auction_image_upload" type="button" value="<?php _e('Select File', 'wdm-ultimate-auction');?> "/>
	</td>
    </tr>
    <?php
    $custom_fields = array();	
	...
	$custom_meta_field = $this->wdm_post_meta('wdm_custom_field');
	...
	for($init=0; $init < $count; $init++){
	    ...
	    {	
		$field_type = "class='required'";	
	    }
	    ?>
	    <tr valign="top">	
	    <th scope="row">	
		<label><?php echo $custom_fields[$init]['label'];?></label>
	    </th>	
	    <td>
		<input type="text" <?php echo $field_type;?> id="wdm_custom_field_<?php echo $init;?>" size="40" name="wdm_custom_field[]" value="<?php echo $custom_meta_field[$init];?>" />
	    </td>
	    </tr>
	    <?php
	}
...
?>
			<!--<option id="<?php echo $key;?>" value="<?php echo $key;?>" <?php if ($this->wdm_post_meta('wdm_payment_method') == $key) {
	echo "selected";
}
...
?>
            <select id="payment_method" name="payment_method">
                <option id="wdm_method_paypal" value="method_paypal" <?php if ($this->wdm_post_meta('wdm_payment_method') == "method_paypal") {
	echo "selected";
}
if (!array_key_exists("method_paypal", $pay_methods)) {
	echo "disabled='disabled'";
}
?>><?php _e("PayPal", "wdm-ultimate-auction");?></option>
                <?php if (!$comm) {
	?>
		<option id="wdm_method_wire_transfer" value="method_wire_transfer" <?php if ($this->wdm_post_meta('wdm_payment_method') == "method_wire_transfer") {
		echo "selected";
	}
	if (!array_key_exists("method_wire_transfer", $pay_methods)) {
		echo "disabled='disabled'";
	}
	?>><?php _e("Wire Transfer", "wdm-ultimate-auction");?></option>
                <option id="wdm_method_mailing" value="method_mailing" <?php if ($this->wdm_post_meta('wdm_payment_method') == "method_mailing") {
		echo "selected";
	}
	if (!array_key_exists("method_mailing", $pay_methods)) {
		echo "disabled='disabled'";
	}
	?>><?php _e("By Cheque", "wdm-ultimate-auction");?></option>
		<option id="wdm_method_cash" value="method_cash" <?php if ($this->wdm_post_meta('wdm_payment_method') == "method_cash") {
		echo "selected";
	}
	if (!array_key_exists("method_cash", $pay_methods)) {
		echo "disabled='disabled'";
	}
	?>><?php _e("Cash", "wdm-ultimate-auction");?></option>
		<?php }?>
	    </select>
	    <?php if ($comm) {
...
if (in_array('administrator', $logged_user_role)) {
	...
	if ($can_add_directly && !isset($_GET['edit_auction']) && !isset($_GET["reactivate"])) {submit_button(__('Save Changes', 'wdm-ultimate-auction'));} elseif ($can_add_directly && isset($_GET["reactivate"])) {submit_button(__('Save Changes', 'wdm-ultimate-auction'));} elseif (isset($_GET['edit_auction']) && !empty($_GET['edit_auction']) && !isset($_GET["reactivate"])) {
	...
	<p>
	    <input type="submit" id="ua-show-payment-btn" class="button-primary" value="<?php _e('Continue with Payment', 'wdm-ultimate-auction');
		echo ' [' . $currency_code . ' ' . $amt . ']';?>" />
