...
class Auctions_List_Table extends WP_List_Table {        
	...
	elseif(isset($_GET["auction_type"]) && $_GET["auction_type"] == "scheduled")
		...
		$row['payment_made'] .= 'Transaction ID: <br /><span class="wdm-mark-hover">'.$listing_trans_id.'</span>';
	    }
	...
	//	require('ajax-actions/send-download-link.php');
		...
		if(!in_array('administrator',$user_det->roles) && ($this->auction_type !="live" && $this->auction_type !="scheduled")){
		   ...
		   $row["bidders"] = "<strong>".__("Bidders", "wdm-ultimate-auction").":</strong><div class='wdm-bidder-list-".$single_auction->ID."'><ul>".$row_bidders."</ul></div>";
		...
		//$check_method = get_post_meta($single_auction->ID, 'wdm_payment_method', true);
			...
			if($winner_bid >= $reserve_price_met)
			{
			    ...
			    //$row['action'] .= "<br /><div id='wdm-send-download-link-".$single_auction->ID."' class='wdm_ua_act_links' style='color:red;cursor:pointer;'>".__('Send Download Link', 'wdm-ultimate-auction')."</div>";
			    if($authorID == $userID)
				require('ajax-actions/send-download-link.php');
		...
		if(!empty($invoiceStat) || get_post_meta($single_auction->ID,'auction_bought_status',true) === 'bought' || (!empty($winner) && in_array('administrator', $winner->roles))){
			...
			if((!empty($winner) && in_array('administrator', $winner->roles))){
			    ...
			    $row['invoice'] .= "<a href='".admin_url("admin.php?page=invoices&payment_type=past&auction=").$single_auction->ID."' target='_blank'>".__('Invoice Details', 'wdm-ultimate-auction')."</a><br /><br />";
		    ...
		    $row['invoice'] .= "<a href='".admin_url("admin.php?page=invoices&payment_type=outstanding&auction=").$single_auction->ID."' target='_blank'>".__('Invoice Details', 'wdm-ultimate-auction')."</a>";
		    if(!empty($winner))
...
<ul class="subsubsub">
    ...
    <li><a href="?page=manage_auctions&auction_type=scheduled" class="<?php echo $manage_auction_tab == 'scheduled' ? 'current' : ''; ?>"><?php _e('Scheduled Auctions', 'wdm-ultimate-auction');?></a>|</li>
