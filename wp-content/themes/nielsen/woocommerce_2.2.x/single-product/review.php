...
?>
<li itemprop="reviews" itemscope itemtype="http://schema.org/Review" <?php comment_class( 'border-2' ); ?> id="li-comment-<?php comment_ID() ?>">
	...
	<div id="comment-<?php comment_ID(); ?>" class="comment_container">
		...
		<div class="comment-meta clearfix">
			...
			<?php else : ?>
				...
				<p class="meta">
					<strong class="author" itemprop="author"><?php comment_author(); ?></strong> <?php

						if ( get_option( 'woocommerce_review_rating_verification_label' ) === 'yes' )
							if ( wc_customer_bought_product( $comment->comment_author_email, $comment->user_id, $comment->comment_post_ID ) )
								echo '<em class="verified">(' . __( 'verified owner', 'yit' ) . ')</em> ';

					?>&ndash; <time itemprop="datePublished" datetime="<?php echo get_comment_date( 'c' ); ?>"><?php echo get_comment_date( __( get_option( 'date_format' ), 'yit' ) ); ?></time>:
				</p>
